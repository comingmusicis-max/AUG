...
ทั้ง 5 งานชี้ `media_dir` ไปที่โฟลเดอร์ `graded` แล้ว — **แต่ยังไม่มีงานไหนถูกเกรดจริง**
ต้องรัน `match_grade.py --write` บนเครื่องก่อน ไม่งั้น `build_draft.py` จะหาไฟล์ไม่เจอ
