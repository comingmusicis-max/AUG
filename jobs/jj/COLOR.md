...
## วิธีที่ง่ายที่สุด — ไฟล์เดียวจบ

โหลด **`jj_grade.py`** ไฟล์เดียว วางที่ไหนก็ได้ (Desktop, Downloads, ไม่สำคัญ) แล้วดับเบิลคลิก

ไม่ต้องโหลด repo ไม่ต้อง Administrator ไม่ต้องวางในโฟลเดอร์ไหนเป็นพิเศษ มันสร้าง
LUT เองลง `%LOCALAPPDATA%\jj-grade\` แล้วส่ง path เต็มให้ Resolve

ก่อนรัน: เปิดโปรเจกต์ `jj` ในหน้า Color เลือก C0001 แล้วเพิ่มโหนดให้ครบ 6
ถ้ายังไม่ครบมันจะบอกเองว่าขาดอีกกี่อัน

ถ้าดับเบิลคลิกแล้วไม่ขึ้นอะไร คลิกขวา → Open with → Python

---

## วิธีเดิม (ต้องมี repo ทั้งก้อน)
